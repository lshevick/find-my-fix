...
const ReviewDetail = ({ id, body, rating, username, service, shopId }) => {
  ...
  const saveNewReview = async() => {
    ...
    const response = await fetch(`/api/v1/shops//`, options).catch(handleError);
    if(!response.ok) {
  ...
  const editReviewDetail = (
      <li className="rounded relative shadow-md bg-base-300 p-2 my-1 flex flex-col items-start w-full">
        ...
        <div>
          <span className="mr-2 font-bold">{username}</span>
          <Rating name="rating" value={newRating} precision={1} />
        </div>
        <div>
          <ul className="flex">
            {service &&
        ...
        <div className="absolute bottom-2 right-2">
          <button type="submit" form="newReviewForm">
            Save Changes
          </button>
        </div>
        </form>
